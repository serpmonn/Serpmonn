#!/usr/bin/env node
/**
 * Typing Short — REAL gameplay:
 * 15s mode, clock jumps to last seconds (still counts down in-engine),
 * human typing until timer hits 0.
 * Act1: choke near 0 → FAIL…
 * Act2: clean finish at 0 → Cleared…
 */
...
const OUT_FINAL = path.join(OUT_DIR, 'typing-preview-en-serpmonn.mp4');
const AUDIO = '/var/www/serpmonn.ru/backend/marketing/assets/audio/games-14.mp3';
const FONT = '/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf';
const TMP = '/tmp/typing-short-rec';
const W = 1080;
const H = 1920;
const FPS = 12;
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${port}/typing/index.html?rec=1&lang=en`, {
    ...
    await run('ffmpeg', [
      ...
      [
        ...
        `drawtext=fontfile=${FONT}:text='Typing…':fontsize=64:fontcolor=white:borderw=4:bordercolor=black@0.85:x=(w-text_w)/2:y=h*0.88:enable='gte(t,${t(markers.typing1)})*lt(t,${t(markers.lowTime)})'`,
        `drawtext=fontfile=${FONT}:text='Time…':fontsize=68:fontcolor=white:borderw=4:bordercolor=black@0.9:x=(w-text_w)/2:y=h*0.88:enable='gte(t,${t(markers.lowTime)})*lt(t,${t(markers.choke)})'`,
        `drawtext=fontfile=${FONT}:text='No…':fontsize=72:fontcolor=0xff5555:borderw=5:bordercolor=black@0.9:x=(w-text_w)/2:y=h*0.88:enable='gte(t,${t(markers.choke)})*lt(t,${t(markers.fail)})'`,
        `drawtext=fontfile=${FONT}:text='FAIL…':fontsize=84:fontcolor=0xff4444:borderw=5:bordercolor=black@0.95:x=(w-text_w)/2:y=h*0.84:enable='gte(t,${t(markers.fail)})*lt(t,${t(markers.failEnd)})'`,
        `drawtext=fontfile=${FONT}:text='Almost…':fontsize=56:fontcolor=white:borderw=4:bordercolor=black@0.9:x=(w-text_w)/2:y=h*0.90:enable='gte(t,${t(markers.fail)})*lt(t,${t(markers.failEnd)})'`,
        ...
        `drawtext=fontfile=${FONT}:text='Cleared…':fontsize=76:fontcolor=white:borderw=5:bordercolor=black@0.9:x=(w-text_w)/2:y=h*0.86:enable='gte(t,${t(markers.win)})*lt(t,${t(markers.winEnd)})'`,
    ...
    await run('ffmpeg', ['-y', '-ss', '1.0', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'typing-firstframe.jpg')]);
    await run('ffmpeg', ['-y', '-ss', `${Math.max(1, Number(t(markers.fail)) + 0.25)}`, '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'typing-fail.jpg')]);
    await run('ffmpeg', ['-y', '-ss', `${Math.max(1, Number(t(markers.win)) + 0.15)}`, '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'typing-win.jpg')]);
