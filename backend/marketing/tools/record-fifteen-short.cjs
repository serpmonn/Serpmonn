#!/usr/bin/env node
/**
 * Fifteen Short — viral near-miss pattern (like minesweeper):
 * Act1: almost solved → fail banner "Almost…"
 * Act2: same setup → Solved… / serpmonn
 * Loopback
 */
...
const OUT_FINAL = path.join(OUT_DIR, 'fifteen-preview-en-serpmonn.mp4');
const AUDIO = '/var/www/serpmonn.ru/backend/marketing/assets/audio/games-12.mp3';
const FONT = '/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf';
const TMP = '/tmp/fifteen-short-rec';
const W = 1080;
const H = 1920;
const FPS = 10;
  ...
  try {
    ...
    await run('ffmpeg', [
      ...
      [
        ...
        `drawtext=fontfile=${FONT}:text='One more…':fontsize=64:fontcolor=white:borderw=4:bordercolor=black@0.85:x=(w-text_w)/2:y=h*0.88:enable='gte(t,${t(markers.oneMore1)})*lt(t,${t(markers.boom)})'`,
    ...
    await run('ffmpeg', ['-y', '-ss', '1.2', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'fifteen-firstframe.jpg')]);
    await run('ffmpeg', ['-y', '-ss', '3.2', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'fifteen-fail.jpg')]);
    await run('ffmpeg', ['-y', '-ss', '6.0', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'fifteen-win.jpg')]);
