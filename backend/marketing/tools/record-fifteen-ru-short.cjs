#!/usr/bin/env node
/**
 * Fifteen RU Short — near-miss pattern:
 * Act1: almost solved → «Почти…»
 * Act2: same setup → «Собрал!»
 * Loopback
 */
...
const OUT_FINAL = path.join(OUT_DIR, 'fifteen-preview-ru-serpmonn.mp4');
const OUT_UPLOAD = path.join(SHORTS_DIR, 'fifteen-vk-ru.mp4');
const AUDIO = '/var/www/serpmonn.ru/backend/marketing/assets/audio/games-12.mp3';
const FONT = '/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf';
const TMP = '/tmp/fifteen-ru-short-rec';
const W = 1080;
const H = 1920;
const FPS = 10;
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${port}/fifteen/index.html?rec=1&lang=ru`, {
    ...
    await page.evaluate(() => window.__rec.setupTwoAway());
    await sleep(1200);
    markers.oneMore1 = frameIdx;
    await page.evaluate(() => window.__rec.moveIndex(14));
    await sleep(900);
    ...
    await run('ffmpeg', [
      ...
      [
        ...
        `drawtext=fontfile=${FONT}:text='Ещё один ход…':fontsize=64:fontcolor=white:borderw=4:bordercolor=black@0.85:x=(w-text_w)/2:y=h*0.88:enable='gte(t,${t(markers.oneMore1)})*lt(t,${t(markers.boom)})'`,
        `drawtext=fontfile=${FONT}:text='Почти…':fontsize=72:fontcolor=white:borderw=4:bordercolor=black@0.9:x=(w-text_w)/2:y=h*0.86:enable='gte(t,${t(markers.boom)})*lt(t,${t(markers.boomEnd)})'`,
        ...
        `drawtext=fontfile=${FONT}:text='Собрал!':fontsize=72:fontcolor=white:borderw=4:bordercolor=black@0.9:x=(w-text_w)/2:y=h*0.86:enable='gte(t,${t(markers.win)})*lt(t,${t(markers.winEnd)})'`,
    ...
    await run('ffmpeg', ['-y', '-ss', '1.2', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'fifteen-ru-firstframe.jpg')]);
    await run('ffmpeg', ['-y', '-ss', '3.2', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'fifteen-ru-fail.jpg')]);
    await run('ffmpeg', ['-y', '-ss', '6.0', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'fifteen-ru-win.jpg')]);
