#!/usr/bin/env node
/**
 * Falling Shapes Short — REAL gameplay from the engine:
 * shapes spawn at top and fall; paddle tracks like a human.
 * Short cut: START ALREADY AT THE END —
 * Act1: late game (high misses) → choke → Game over
 * Act2: quick restart → catch streak → Caught…
 */
...
const OUT_FINAL = path.join(OUT_DIR, 'redsquare2-preview-en-serpmonn.mp4');
const AUDIO = '/var/www/serpmonn.ru/backend/marketing/assets/audio/games-13.mp3';
const FONT = '/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf';
const TMP = '/tmp/rs2-short-rec';
const W = 1080;
const H = 1920;
const FPS = 12;
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${port}/redsquare2/index.html?rec=1&lang=en`, {
    ...
    await run('ffmpeg', [
      ...
      [
        ...
        `drawtext=fontfile=${FONT}:text='One left…':fontsize=64:fontcolor=white:borderw=4:bordercolor=black@0.85:x=(w-text_w)/2:y=h*0.88:enable='gte(t,${t(markers.act1)})*lt(t,${t(markers.choke)})'`,
        ...
        `drawtext=fontfile=${FONT}:text='FAIL…':fontsize=84:fontcolor=0xff5555:borderw=5:bordercolor=black@0.95:x=(w-text_w)/2:y=h*0.84:enable='gte(t,${t(markers.fail)})*lt(t,${t(markers.failEnd)})'`,
        ...
        `drawtext=fontfile=${FONT}:text='Caught…':fontsize=76:fontcolor=white:borderw=5:bordercolor=black@0.9:x=(w-text_w)/2:y=h*0.86:enable='gte(t,${t(markers.win)})*lt(t,${t(markers.winEnd)})'`,
    ...
    await run('ffmpeg', ['-y', '-ss', '1.2', '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'redsquare2-firstframe.jpg')]);
    await run('ffmpeg', ['-y', '-ss', `${Math.max(1, Number(t(markers.fail)) + 0.25)}`, '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'redsquare2-fail.jpg')]);
    await run('ffmpeg', ['-y', '-ss', `${Math.max(1, Number(t(markers.win)) + 0.15)}`, '-i', OUT_FINAL, '-update', '1', '-frames:v', '1', path.join(OUT_DIR, 'redsquare2-win.jpg')]);
