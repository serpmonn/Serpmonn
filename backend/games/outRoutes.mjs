...
// Клики пишутся в MySQL (affiliate_clicks).

import {
  ensureAffiliateClickTables,
  logAffiliateClick,
  getAffiliateClickStats,
} from './affiliateClicks.model.mjs';
...
export function outRoutes(app) {
  ensureAffiliateClickTables().catch((err) => {
    console.error('[affiliate/out] Failed to ensure tables:', err?.message || err);
  });
  ...
  app.get('/out', (req, res) => {
    ...
    setImmediate(() => {
      logAffiliateClick(game, ip, ua).catch((e) => {
  ...
  app.get('/out/stats', async (req, res) => {
    try {
