...
async function pollThreadMessages(modal, username) {
  ...
  if (messagesFingerprint(messages) === messagesFingerprint(inboxState.messages)) return;
  const stickToBottom = isInboxNearBottom(modal);
  inboxState.messages = messages;
  const listEl = modal.querySelector('[data-inbox-list]');
  if (!listEl) return;
  listEl.innerHTML = renderChatThread(messages, (key, vars) => tk(key, vars));
  bindChatThread(modal, username);
...
function startThreadPoll(modal, username) {
  stopThreadPoll();
  if (!modal || !username) return;
  threadPollTimer = setInterval(() => {
    void pollThreadMessages(modal, username);
  }, 3000);
}
...
async function loadInboxInto(modal) {
  ...
  if (inboxState.view === 'thread' && inboxState.username) {
    ...
    onInboxRead?.();
    await updateActivityTabBadges(modal);
    startThreadPoll(modal, inboxState.username);
