...
async function refreshAppTabBadges() {
  try {
    const [dmRes, feedRes] = await Promise.all([
      spnFetch('/api/dm/unread-count', { credentials: 'include' }),
      spnFetch('/api/findings/notifications/unread-count', { credentials: 'include' }),
    ]);
...
setInterval(refreshAppTabBadges, 15000);
refreshAppTabBadges();
