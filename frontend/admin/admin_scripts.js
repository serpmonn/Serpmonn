...
function roleLabel(r) {
    const map = {
        admin: 'Администратор', moderator: 'Модератор', editor: 'Редактор',
        support: 'Поддержка', developer: 'Разработчик', marketer: 'Маркетолог', tester: 'Тестировщик',
    };
    return map[r] || r;
}
...
function autoFillMailLocal() {
    ...
    if (!mailLocalInput || !document.getElementById('createMailbox')?.checked) return;
...
function applyFilters() {
    ...
    filtered = allEmployees.filter(e => {
        const matchQ = !q || [e.first_name, e.last_name, e.email, e.mailbox, e.position, e.role]
    ...
    if (page.length === 0) {
        ...
        tbody.innerHTML = page.map(e => `
            <tr>
                ...
                <td>${esc(e.email)}</td>
                <td>${e.mailbox ? `<span style="font-size:13px;color:#059669">${esc(e.mailbox)}</span>` : '<span style="color:#9ca3af">—</span>'}</td>
                ...
                <td>
                    <div class="actions-cell">
                        ...
                        <button class="btn-icon delete" title="Удалить" onclick="openDelete('${e.id}', '${esc(e.first_name)} ${esc(e.last_name)}', ${JSON.stringify(e.mailbox || null)})">🗑</button>
                    </div>
                </td>
            </tr>
        `).join('');
...
async function loadEmployees() {
    document.getElementById('employeesTbody').innerHTML =
        '<tr><td colspan="8" class="loading-cell">Загрузка...</td></tr>';
    try {
        ...
        document.getElementById('employeesTbody').innerHTML =
            `<tr><td colspan="8" class="empty-cell">Ошибка загрузки: ${esc(err.message)}</td></tr>`;
    }
}
...
// --- MODAL: DELETE ---
window.openDelete = function(id, name, mailbox) {
    deleteTargetId = id;
    document.getElementById('deleteConfirmText').textContent =
        `Удалить сотрудника «${name}»? Это действие необратимо.`;
    document.getElementById('deleteMailboxCheck').checked = false;
    const mailboxRow = document.getElementById('deleteMailboxRow');
    if (mailbox) {
        document.getElementById('deleteMailboxAddr').textContent = mailbox;
        mailboxRow.style.display = 'block';
    } else {
...
async function saveEmployee(e) {
    ...
    if (needMailbox) {
        ...
        body.mailbox = `${mailLocal}@serpmonn.ru`;
    }
