...
function showDeathAdOnce() {
  if (adShownThisDeath) return;
  if (typeof window.showFullScreenAd !== 'function') return;
  adShownThisDeath = true;
  try {
...
function bindUi() {
  ...
  // msgLeaderboard stripped for Yandex Games build.
  if (ui.msgDonate) {
  ...
  // Leaderboard navigation stripped for Yandex Games build.
...
function showMsg(title, text, onOk, extra) {
  ...
  const showLb = false; // site leaderboard stripped in Yandex Games build
  if (ui.msgLeaderboard) ui.msgLeaderboard.hidden = true;
  if (ui.msgDonate) ui.msgDonate.hidden = true;
  if (ui.msgOk) {
    ui.msgOk.textContent = t('ui.ok');
...
function openNeliLeaderboardPage() {
  // Site score_table navigation removed for Yandex Games.
  quitDemoToTitle();
}
...
function submitNeliScore() {
  // Site POST /add-score removed for Yandex Games build.
...
if (/[?&]rec=1(?:&|$)/.test(location.search)) {
  window.__rec = {
    getState() {
      const onTitle = !!(ui.title && !ui.title.hidden);
      const dead = mode === 'msg' && !!(msgSource && msgSource.deathReason);
      const px = player ? player.x + player.w / 2 : null;
      const py = player ? player.y + player.h / 2 : null;
      const interactables = (scene && scene.interactables
        ? scene.interactables
            .filter((it) => !it.done)
            .map((it) => ({
              id: it.id || null,
      ...
      const npcs = (scene && scene.npcs
        ...
        : []);
      return {
        screen: onTitle ? 'title' : (mode || 'play'),
        mode: mode || null,
        alive: !dead && !onTitle,
        sceneId: scene && scene.id ? scene.id : null,
        paused: !!paused,
        player: px == null ? null : { x: px, y: py },
        interactables,
        npcs,
        dialogueOpen: mode === 'dialogue',
        msgOpen: !!(ui.msg && !ui.msg.hidden),
      };
    },
    startDemo,
    startGame,
    ...
    moveToward(tx, ty) {
      if (!player || mode !== 'play' || paused) return false;
      const cx = player.x + player.w / 2;
      const cy = player.y + player.h / 2;
      const dx = tx - cx;
      const dy = ty - cy;
      ...
      if (Math.abs(dx) >= Math.abs(dy)) {
        if (dx > 6) keys.ArrowRight = true;
        else if (dx < -6) keys.ArrowLeft = true;
      } else if (dy > 6) keys.ArrowDown = true;
      else if (dy < -6) keys.ArrowUp = true;
      if (keys.ArrowUp) keys.KeyW = true;
      if (keys.ArrowDown) keys.KeyS = true;
      if (keys.ArrowLeft) keys.KeyA = true;
      if (keys.ArrowRight) keys.KeyD = true;
      if (dist > 70) {
        keys.ShiftLeft = true;
      }
      return false;
    ...
    advanceDialogue() {
      ...
      if (!dialogueTapReady()) return false;
      if (ui.dlgChoices && ui.dlgChoices.children.length) {
        const btn = ui.dlgChoices.querySelector('button');
        if (btn) btn.click();
        return true;
      }
      advanceDialogue();
    ...
    dismissMsg() {
      if (ui.msg && !ui.msg.hidden && ui.msgOk) {
        ui.msgOk.click();
        return true;
      }
      return false;
    ...
    talkBoss() {
      if (!scene || mode !== 'play') return false;
      if (player) {
        player.x = 540;
        player.y = 200;
      }
      const it = (scene.interactables || []).find((x) => x.id === 'boss-talk' && !x.done);
      if (it) it.done = true;
      // fire-and-forget dialogue (do not return the Promise)
      runDialogue([
        {
          speaker: 'Начальник полиции',
          right: 'boss',
    ...
    triggerVan() {
      if (!scene || scene.id !== 'road') return false;
      if (window.__rec._vanShown) return false;
      window.__rec._vanShown = true;
      if (player) {
        player.x = 330;
        player.y = 430;
      }
      setQuest('quest.woods');
      // fire-and-forget; never return the Promise to Playwright
      runDialogue([
        { speaker: '', text: 'Странный фургон сворачивает с дороги — прямо в чащу.' },
        { speaker: '', text: 'В «Волчий лес». Без единой нормальной дороги. Просто так туда не ездят.' },
        { speaker: '', text: 'Ты решаешь проследовать за ним.' },
      ]);
      return true;
    },
    /** Auto-advance dialogues while recording promo. */
    tickAutoDialogue(minMs = 1100) {
      if (mode !== 'dialogue') return false;
      if (!window.__rec._dlgAt) window.__rec._dlgAt = performance.now();
      if (performance.now() - window.__rec._dlgAt < minMs) return false;
      if (!dialogueTapReady()) return false;
      window.__rec._dlgAt = performance.now();
      if (ui.dlgChoices && ui.dlgChoices.children.length) {
        const btn = ui.dlgChoices.querySelector('button');
        if (btn) btn.click();
        return true;
...
boot()
  .then(() => {
    if (typeof window.__ygBoot === 'function') return window.__ygBoot();
  })
  .catch((err) => {
    console.error(err);
