...
const primaryMenuPath = resolvedDir ? `/frontend/${resolvedDir}/menu2.html` : '/frontend/menu2.html';
fetch(primaryMenuPath)
    .then(r => r.ok ? r : fetch('/frontend/menu2.html'))
            ...
            anchors.forEach(a => {
                ...
                if (currentLang === 'ru') {
                    if (rest.length === 0 || (rest.length === 1 && /^(index\.html)?$/.test(rest[0]))) {
                        a.setAttribute('href', '/frontend/main2.html');
                        return;
                    }
