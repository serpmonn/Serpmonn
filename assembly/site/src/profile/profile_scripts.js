...
import { t } from '../scripts/i18n-loader.js';
    ...
    try {
      ...
      if (currentPlan === 'pro') {
        ...
        planStatusEl.textContent = until
          ? t('profile.proActiveUntil').replace('{date}', until)
      ...
      if (currentPlan === 'pro' && data.quotas && data.quotas.pro_monthly) {
        ...
        planQuotaHintEl.textContent =
          q.used === 0
            ...
            : t('profile.proQuotaUsed').replace('{used}', q.used);
  ...
  async function updateReferralCounter() {
    ...
    try {
      ...
      referralCounterEl.textContent =
        referralCount > 0
          ? t('profile.referralCount').replace('{count}', referralCount)
  ...
  if (proExchangeDaysInput && proExchangeSubmit) {
    proExchangeSubmit.addEventListener('click', async () => {
      ...
      const ok = window.confirm(
        t('profile.exchangeConfirm').replace('{points}', needPoints).replace('{days}', days)
      );
      if (!ok) return;
      ...
      try {
        ...
        setGlobalMessage(
          data?.message || t('profile.proExtended').replace('{days}', days).replace('{word}', dayWord),
          'success'
        );
